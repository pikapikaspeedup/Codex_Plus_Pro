...
- 桌面宠物的未读通知批量处理与关闭状态持久化
- 设置窗口可独立开关主题、宠物、画中画和便捷模型选择
- `打开原版 Codex.app` 一键停止注入并恢复官方启动方式

## 桌面宠物通知

当宠物上方出现多条 `Ready` 未读结果时，顶部会显示两个操作：

- `全部已读`：通过 Codex 自己的已读接口更新这些任务，再关闭对应卡片。
- `清空未读`：只清空当前这些 `Ready` 卡片，不改变任务的未读状态。

单独关闭一条卡片或使用 `清空未读` 后，项目会按“通知 ID + 本轮 turnKey”保存在 Codex 本地存储中。下次启动宠物时，同一轮结果不会再次悬浮；任务产生新一轮结果后，因为 turnKey 已变化，新通知仍会正常出现。

批量操作不会隐藏正在运行、等待授权或需要输入的通知。
...
├── Codex Plus Pro.app
├── 打开原版 Codex.app
└── Codex-Plus-Pro-1.7.0-macOS.zip
```
